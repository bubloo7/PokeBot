...
class PokeBot(Player):
    ...
    def choose_move(self, battle):
        ...
        if battle.dynamax_turns_left is None:
            ...
            for m in battle.available_moves:
                ...
                if m.id == glare.id and battle.opponent_active_pokemon.status is None:
                    immune = False
                ...
                # checks if the opponent's pokemon doesn't have another status
                if m.id == twave.id and battle.opponent_active_pokemon.status is None:
                    immune = False
                    # checks if the enemy pokemon is immune
                    for type in battle.opponent_active_pokemon.types:
                ...
                # checks if the opponent's pokemon doesn't have another status
                if m.id == toxic.id and battle.opponent_active_pokemon.status is None:
                    immune = False
                ...
                # checks if the opponent's pokemon doesn't have another status
                if m.id == wisp.id and battle.active_pokemon.status is None:
                    immune = False
        ...
        if (battle.active_pokemon.fainted or battle.active_pokemon.boosts["atk"] <= -2 or battle.active_pokemon.boosts[
            "spa"] <= -2) and len(battle.available_switches) > 0:
        ...
        if battle.available_moves:
            ...
            for i in range(len(battle.available_moves)):
                ...
                damages[i] = damageCalc(battle.active_pokemon, battle.opponent_active_pokemon,
                                        battle.available_moves[i], battle.weather, battle.fields)
            index = 0
            max = 0
            ...
            # The last pokemon standing always dynamaxes
